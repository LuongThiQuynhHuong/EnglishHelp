...
describe('vocabulary validation and search', () => {
  it('accepts web image URLs without file extensions and does not check reachability', () => {
    ...
    expect(validateVocabularyInput({ word: 'apple', vietnameseMeaning: 'táo', englishMeaning: 'fruit', imageUrl: 'https://images.example.com/preview?id=42' })).toBeNull();
  ...
  it('normalizes Vietnamese search and duplicate content while allowing distinct meanings', () => {
    ...
    expect(cleanVocabularyInput({ word: '  ice   cream ', vietnameseMeaning: ' kem ', englishMeaning: ' food ', imageUrl: ' ' }).imageUrl).toBeNull();
  });
});
